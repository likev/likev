...
/*
	...
	2.启动追踪 contr::contour_track::begin_actions()
...
namespace contr{
	...
	{
		double rate;
		bool is_iso_point;   //在此边上是否有等值点
	};

	struct iso_line_info     //等值线的信息
	{
		std::vector<iso_point> a_iso_line;
	...
	public:
		...
		typedef std::vector<iso_line_info> iso_lines_type;
	private:
		int n_level; //等值线数
		std::vector<double> contr_level;
		...
		double cur_flw_value;       //当前正在追踪的等值线值
		iso_point pre_iso_point, cur_iso_point, next_iso_point;

		iso_line_info now_iso_line;    //当前等值线
		iso_lines_type all_iso_line;     //所有等值线
	...
	public:
		...
		iso_lines_type get_all_iso_line()
		{
			return all_iso_line;
		}
	...
	private:
            ...
            */
			...
			for (size_t i = 0; i < edge.size(); i++)
			{
				for (size_t j = 0; j < edge[i].size(); j++)
				{
					...
					{
						edge[i][j].rate = -2.0;
						edge[i][j].is_iso_point = false;
					}
					else
					{
						double flag = (cur_flw_value - h0) * (cur_flw_value - h1);
						...
						{
							edge[i][j].rate = (cur_flw_value - h0) / (h1 - h0);
							edge[i][j].is_iso_point = true;
						}
						else //与其中一个相等
						{
							//修正
							if (beql(cur_flw_value, h0))
								h0 += Excursion;
							else
								h1 += Excursion;
		...
		void tracing_non_closed_contour()
		{
            ...
            {
				if (x_side[grid_rows-1][j].is_iso_point)
                {
            ...
            for (int i = 0; i < grid_rows-1; i++)
            {
				if (y_side[i][grid_cols-1].is_iso_point)
                {
		...
		{
			now_iso_line.a_iso_line.clear();
			now_iso_line.value = cur_flw_value;
			...
			if(is_h)
			{
				x_side[row][col].is_iso_point = false;
			}
			else
			{
				y_side[row][col].is_iso_point = false;
			}
        ...
        void trace_next_point()
        {
			...
			if(left_edge.is_iso_point)
			{
				if(oppsite_edge.is_iso_point)
				{
			...
			else if(oppsite_edge.is_iso_point)
			{
        ...
        void set_default_level()
		{
            ...
            for (int i = 0; i < n_level; i++)
            {
				contr_level.push_back((grdf.zmax-grdf.zmin)*i/(n_level-1)+grdf.zmin );
            }
        }
		...
		void calc_coord(int row,int col,bool is_h)
		{
			...
			now_iso_line.a_iso_line.push_back(iso_point(row,col,is_h,x,y));
