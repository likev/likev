/*处理gprs实时雨量或查询雨量表格的一行
  最后修改 2010 8 23 17:20

  1.将last_time修改为last_time_str
  2.修改last_time_str的查找方式
*/
...
class GprsRainLine
{
	std::string station_id, station_name, gprs_id, last_time_str;
	double value;
...
public:
	...
	void deal_line_str(const std::string &line_str,bool realtime)
	{
		...
		{
			pos1 = line_str.find("<td",++pos1);
			last_time_str = get_tag_content(line_str.substr(pos1), "td");
		}
...
private:
	...
	std::string webserver,weburl,last_time_str,file_name, file_content;
...
private:
	...
	void deal_record()
	{
		...
		pos2 = file_content.find("|",pos1);

		last_time_str = file_content.substr(pos1,pos2-pos1);
		replace_all(last_time_str,"&nbsp;"," ");
...
public:
	...
	void update_js_file(std::string file,bool realtime=true)
	{
		...
		update_content += "var recordTime = '"+last_time_str+"';\n\n";
