...
public:
	...
	void set_r_gate_count(unsigned rcount)
	{
		...
		r.resize(360, std::vector<double>(rcount, VALUE_INVALID));
	}

	void set_vw_gate_count(unsigned rcount)
	{
		vw_gate_count = rcount;
		v.resize(360, std::vector<double>(rcount, VALUE_INVALID));
		w.resize(360, std::vector<double>(rcount, VALUE_INVALID));
	}
