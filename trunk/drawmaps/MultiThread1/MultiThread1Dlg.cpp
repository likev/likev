...
BEGIN_MESSAGE_MAP(CMultiThread1Dlg, CDialog)
	...
	ON_BN_CLICKED(IDC_BUTTON1, &CMultiThread1Dlg::OnBnClickedButton1)
	ON_MESSAGE(WM_DISPLAY,OnDisplay)
END_MESSAGE_MAP()


// CMultiThread1Dlg 消息处理程序
...
{
	CMultiThread1Dlg *p = (CMultiThread1Dlg *)pParam;

	CEdit * pEdit = (CEdit *)p->GetDlgItem(IDC_NUMBER);
	CString number_str;
	number_str.Format("%d",++(p->thread_number));
	pEdit->SetWindowTextA(number_str);
...
void CMultiThread1Dlg::OnBnClickedButton1()
{
	// TODO: 在此添加控件通知处理程序代码
	m_pCalculateThread=(CCalculateThread*)AfxBeginThread(RUNTIME_CLASS(CCalculateThread));	
	Sleep(500);	
	m_pCalculateThread->PostThreadMessage(WM_CALCULATE,20,NULL);
...
{	
	...
	SetDlgItemInt(IDC_NUMBER,nTemp,FALSE);  
	return 0;
}